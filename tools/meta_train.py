...
def train(cfg, local_rank, distributed, logger):
    ...
    meta_net = MLP(hidden_size=100, num_layers=1).to(device='cuda')  # weight function
    meta_lr = 0.01
    meta_optimizer = torch.optim.SGD(meta_net.parameters(), lr=meta_lr, momentum=0.9, weight_decay=0.01)
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        model.train()
        fix_eval_modules(eval_modules)
        pseudo_model = build_detection_model(cfg)
        ...
        # pseudo train
        pseudo_losses = pseudo_model(images, targets)
        pseudo_loss_rel = pseudo_losses['loss_rel']
        pseudo_weight = meta_net(pseudo_loss_rel)
        pseudo_loss_rel = pseudo_weight * pseudo_loss_rel
        pseudo_model.zero_grad()
        pseudo_grads = torch.autograd.grad(pseudo_loss_rel, pseudo_model.parameters(), create_graph=True, allow_unused=True)
        ...
        for val_data_loader in val_data_loaders:
            for _, (images_val, targets_val, _) in enumerate(val_data_loader):
        ...
        # losses['loss_rel']即是relation_head的loss，是由谓词标签计算交叉熵得到的损失
